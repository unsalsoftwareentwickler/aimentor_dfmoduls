<?php
// omnichannel.php - İleri Besleme Eğitimi Değerlendirmesi
...
eğitim konusuda aşağıda bu konunun dışına çıkmadan, kullanıcıdan "Sence ileri besleme ve geri bildirim arasında ne fark vardır?" sorusuna kısa bir yanıt aldıktan sonra aşağıdaki konuyu özetle anlat:
...
Eğitim Metni Sonu'],
    ...
    if (isset($responseData['choices'][0]['message']['content'])) {
        ...
        echo json_encode(['error' => 'API cevabı beklenmedik çıktı.', 'raw' => $response]);
    }
    exit;
}
?>
...
<head>
  ...
  <title>AkademiMentor - İleri Besleme Değerlendirmesi</title>
  <style>
    :root{
      --chat-bg:#fff;
      --chat-border:#e6e8eb;
    ...
    .explanation-box.correct::before {
      content: "✅";
    }
    ...
    .explanation-box.incorrect::before {
      content: "❌";
    }
    ...
    .send-btn{
      ...
      border-radius:12px; cursor:pointer; font-weight:600; font-size: 0.75em;
    }
...
<body>
      ...
      <section class="chat-panel">
        <header class="chat-header">
          <div class="title-group">
            ...
            <div class="title">AkademiMentor - İleri Besleme Değerlendirmesi</div>
          </div>
        ...
        <div class="chat-input">
          <input type="text" id="userInput" placeholder="Eğitim hakkında sorularınızı yazın..." />
  ...
  <script>
    ...
    const questions = [
      {
        question: "İleri besleme (feedforward) yaklaşımının temel özelliği nedir?",
        options: [
          "Geçmiş hatalarını analiz etmeye odaklanır",
          "Gelecekte nasıl daha iyi olunabileceğine odaklanır"
        ],
        correct: 1,
        explanation: "İleri besleme, geçmiş davranışları değerlendirmek yerine gelecekte nasıl daha iyi olunabileceğine odaklanan bir gelişim yaklaşımıdır."
      },
      {
        question: "GROW modeli hangi aşamalardan oluşur?",
        options: [
          "Goal (Hedef), Reality (Gerçeklik), Options (Seçenekler), Will (İrade)",
          "Growth (Büyüme), Reflection (Yansıma), Output (Çıktı), Wisdom (Bilgelik)"
        ],
        correct: 0,
        explanation: "GROW modeli Goal (Hedef), Reality (Gerçeklik), Options (Seçenekler), Will/Way Forward (İrade/İleriye Dönük Plan) aşamalarından oluşur."
      },
      {
        question: "İleri besleme verirken hangi kelime türünü kullanmak önerilir?",
        options: [
          "Siyah kelimeler (hata, yanlış, eksik gibi)",
          "Beyaz kelimeler (gelişim, iyileştirme, potansiyel gibi)"
        ],
        correct: 1,
        explanation: "İleri besleme verirken 'beyaz kelimeler' (gelişim, iyileştirme, potansiyel gibi) kullanmak önerilir. Siyah kelimeler (hata, yanlış, eksik) savunma mekanizmasını tetikleyebilir."
      }
    ];

    function addMessage(from, text, isQuestion = false, questionIndex = -1) {
      ...
      if (isQuestion && questionIndex >= 0) {
        messageContent += `<div class="options-container" data-question-index="${questionIndex}">`;
        questions[questionIndex].options.forEach((option, index) => {
          messageContent += `
            <div class="option-button" onclick="selectAnswer(${questionIndex}, ${index})">
    ...
    function addExplanationMessage(from, text, isCorrect) {
      const messageDiv = document.createElement('div');
      messageDiv.className = `message ${from === 'Sen' ? 'user' : 'ai'}`;
    ...
    function selectAnswer(questionIndex, selectedOption) {
      const currentQuestionContainer = document.querySelector(`[data-question-index="${questionIndex}"]`);
      const optionButtons = currentQuestionContainer.querySelectorAll('.option-button');
      ...
      optionButtons[selectedOption].classList.add(isCorrect ? 'correct' : 'incorrect');
      ...
      if (isCorrect) {
        setTimeout(() => {
          addExplanationMessage("AkademiMentor", `🎉 Tebrikler! Doğru cevap. 
...
${question.explanation}`, true);
        ...
        setTimeout(() => {
          addExplanationMessage("AkademiMentor", `Yanlış cevap. Doğru cevap: ${String.fromCharCode(65 + question.correct)}) ${question.options[question.correct]}
...
${question.explanation}`, false);
    ...
    function askQuestion() {
      ...
      addMessage("AkademiMentor", `Soru ${currentQuestionIndex + 1}: ${question.question}`, true, currentQuestionIndex);
    }

    function startQuiz() {
      quizStarted = true;
      currentQuestionIndex = 0;
      addMessage("AkademiMentor", "Harika, başarılar! 🚀");
      setTimeout(() => {
        askQuestion();
      }, 1500);
    }
    ...
    function completeQuiz() {
      quizCompleted = true;
      addMessage("AkademiMentor", "Şimdi sıra sende! Eğitimi bitirdiğin için kapatabilir veya eğitim hakkında dilediğin kadar soru sorabilirsin.");
    }

    addMessage("AkademiMentor", `İleri Besleme eğitimini tamamladığın için tebrikler! 

Şimdi sana 3 sorum var, her bir sorunun iki şıkkı var.

Hazırsan "Evet" yazabilirsin!`);
